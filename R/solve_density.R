...
#' note that E(t) is indepedent of tree structure
#' solve differential equations using Runge-Kutta methods
#'
...
integrate_E <- function(f_initial, T_vector, argument, d_t, non_negativity_cutoff) 
{
  ...
  f <- f_initial              # initial E at time zero, a vector of non-sampled probablities with the length of the types
...
#' calculate the derivative of Extinction density 
#'
...
derivative_E <- function(phi, argument)
{
  ...
  dp[x_1] <- -(b[x_1]+d[x_1]+mu[x_1])*phi[x_1] + d[x_1] + (b[x_1]+mu[x_1])*phi[x_1]*phi[x_1]    # no mutation for the last state, the "mutation" does not change the type
  return (dp)
}
...
#' integrate_D is a function to solve differential equations using Runge-Kutta methods
#' D(t) can be viewed as the density that a node at state i leads to the sampled tree
...
#' @param T_vector time discretization
#' @param argument a list of birth, death and mutation data
#' @param E_list precalculated E(t), acorss a list of different types
...
integrate_D <- function(f_initial, T_vector, argument, E_list, ini_rank, t_1, d_t, non_negativity_cutoff) 
{
  ...
  for (ti in 2:x_1) {
    ...
    eta=replicate(fitness_count, 0)  # this is the pre-calculated Ei for the current time
    for (j in 1:fitness_count) {
      eta[j]=E_list[[j]](tnext+t_1)
    }
    ## this step can be refined
    
    while (t < tnext){
      h <- min(d_t, tnext-t)
      k1 <- derivative_D(f, eta, argument, ini_rank)
      k2 <- derivative_D(f+0.5*h*k1, eta, argument, ini_rank)
      k3 <- derivative_D(f+0.5*h*k2, eta, argument, ini_rank)
      k4 <- derivative_D(f+h*k3, eta, argument, ini_rank)
      t <- t+h
      f <- f+h/6*(k1+2*k2+2*k3+k4)
      f[f<non_negativity_cutoff] <- non_negativity_cutoff
    }
...
#' @param argument a list of birth, death and mutation data
#' @param ini_rank only keeps the possible transition between states
...
derivative_D <- function(phi, eta, argument, ini_rank) 
{
  ...
  dp[x_1] <- ini_rank[x_1]*(-(b[x_1]+d[x_1]+mu[x_1])*phi[x_1] + 2*(b[x_1]+mu[x_1])*phi[x_1]*eta[x_1]) # no mutation for the last state
  return (dp)  
}
...
get_D_list <- function(rho, T_vector, d_t, argument, E_list, t_1, non_negativity_cutoff) {
  ...
  for (i in 1:fitness_count) {            # for each fitness type (CHILD)
    ...
    if (t_1==0){                          # at a leaf, set the matching type with sampling probability
      phi0[i] <- rho                      
    } else {                              # at an internal node, set the matching type with 1
      phi0[i] <- 1
    }
...
#' calculate the propagator for a branch
#'
#' @param rho sampling probability
#' @param argument a list of birth, death and mutation data
